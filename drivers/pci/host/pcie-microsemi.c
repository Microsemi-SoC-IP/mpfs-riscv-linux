...
static int microsemi_pcie_init_irq_domain(struct microsemi_pcie_port *port)
{
	...
	bridge_base_addr = port->axi_base_addr + ((port->bridge == 0) ?
		MICROSEMI_PCIE0_BRIDGE_ADDR : MICROSEMI_PCIE1_BRIDGE_ADDR);
...
static int microsemi_pcie_init_port(struct microsemi_pcie_port *port)
{
	...
	if (port->bridge == 0) {
		bridge_base_addr = port->axi_base_addr
			+ MICROSEMI_PCIE0_BRIDGE_ADDR;
		ctrl_base_addr = port->axi_base_addr
